...
function successCmd(cmdSent) {
  return cmdSent ? (
    <span class="cmd-status-success">&#10003;</span>
  ) : (
    <p>No Cmds Sent Yet</p>
  );
}

function failedCmd(cmdSent) {
  return cmdSent ? (
    <span class="cmd-status-failure">&#10007;</span>
  ) : (
    <p>No Cmds Sent Yet</p>
  );
}

const whichStatus = ({ posting, success, cmdSent }) => {
  switch (posting) {
    case true:
      return (
        <img
    ...
    default:
      return success ? successCmd(cmdSent) : failedCmd(cmdSent);
  }
};

export default (state) => (
  <div style={{ height: '40px' }}>{whichStatus(state)}</div>
);
